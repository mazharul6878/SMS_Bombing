...
async def _main():
    ...
    print(f"{_c['y']}⚠️  WARNING{_c['e']}")
    print(f"{_c['r']}{'-'*60}{_c['e']}")
    print("This tool is only for educational purposes.")
    print("If used for illegal activities, you are responsible.")

    choice = input(f"\n{_c['c']}Agree? (Y to continue): {_c['w']}").strip().upper()
    if choice != "Y":
        print(f"{_c['r']}Exiting...{_c['e']}")
        return
    ...
    while True:
        ...
        if choice == "3":
            print(f"{_c['r']}Exit selected{_c['e']}")
            break

        elif choice == "2":
            _state['paused'] = not _state['paused']
            print(f"{_c['y']}Paused: {_state['paused']}{_c['e']}")
            await asyncio.sleep(1)
            continue

        elif choice != "1":
            print(f"{_c['r']}Invalid option{_c['e']}")
            await asyncio.sleep(1)
            continue
        ...
        while True:
            phone = input(f"\n{_c['g']}Enter phone: {_c['w']}").strip()
            if phone and len(phone) >= 10:
                break
            print(f"{_c['r']}Invalid number{_c['e']}")
        ...
        print(f"\n{_c['g']}Started... Press CTRL+C to stop{_c['e']}\n")
